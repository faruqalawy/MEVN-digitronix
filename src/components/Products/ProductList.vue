<template>
  <div v-bind="$attrs">
    ...
    <div class="custom-display my-8" v-if="category">
      ...
      <h3 class="text-h3 mb-2.5 lg:text-h5">{{ ProductsLaptopList.length }} Products</h3>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-x-3 md:gap-x-5 gap-y-5 md:gap-y-12">
        <div v-for="product in ProductsLaptopList" :key="product.id">
          ...
          >
            ...
            <h3 class="text-h3 md:text-h5">{{ product.name }}</h3>
            <h3 class="text-h3 text-brightYellow md:text-h5 md:mt-1">
